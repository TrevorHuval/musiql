...
public class CompilerTests
{
    ...
    public void Numeric_comparison()
    {
        ...
        Assert.Equal(
            """
            SELECT * FROM ((SELECT r.mbid AS id, r.name AS title, a.name AS artist, rg.name AS album, r.first_release_year AS year, r.length_ms AS length_ms, coalesce(rp.score, 0) AS popularity FROM catalog.recording r JOIN catalog.artist a ON a.id = r.artist_id LEFT JOIN catalog.release_group rg ON rg.id = r.release_group_id LEFT JOIN catalog.recording_popularity rp ON rp.recording_id = r.id WHERE r.first_release_year >= @p0 AND rp.score IS NOT NULL ORDER BY rp.score DESC NULLS LAST LIMIT @row_limit) UNION ALL (SELECT r.mbid AS id, r.name AS title, a.name AS artist, rg.name AS album, r.first_release_year AS year, r.length_ms AS length_ms, coalesce(rp.score, 0) AS popularity FROM catalog.recording r JOIN catalog.artist a ON a.id = r.artist_id LEFT JOIN catalog.release_group rg ON rg.id = r.release_group_id LEFT JOIN catalog.recording_popularity rp ON rp.recording_id = r.id WHERE r.first_release_year >= @p0 AND rp.score IS NULL LIMIT @row_limit)) ranked LIMIT @row_limit
            --
            @p0 = bigint 1990
            @row_limit = int 10
            """,
    ...
    public void String_equality_is_case_insensitive()
    {
        ...
        Assert.Equal(
            """
            SELECT * FROM ((SELECT a.mbid AS id, a.name AS name, a.begin_year AS year, coalesce(ap.listeners, 0) AS popularity FROM catalog.artist a LEFT JOIN catalog.artist_popularity ap ON ap.artist_id = a.id WHERE lower(a.name) = lower(@p0) AND ap.listeners IS NOT NULL ORDER BY ap.listeners DESC NULLS LAST LIMIT @row_limit) UNION ALL (SELECT a.mbid AS id, a.name AS name, a.begin_year AS year, coalesce(ap.listeners, 0) AS popularity FROM catalog.artist a LEFT JOIN catalog.artist_popularity ap ON ap.artist_id = a.id WHERE lower(a.name) = lower(@p0) AND ap.listeners IS NULL LIMIT @row_limit)) ranked LIMIT @row_limit
            --
            @p0 = text "Nirvana"
            @row_limit = int 100
            """,
    ...
    public void Contains_escapes_wildcards_into_a_bound_pattern()
    {
        ...
        Assert.Equal(
            """
            SELECT * FROM ((SELECT r.mbid AS id, r.name AS title, a.name AS artist, rg.name AS album, r.first_release_year AS year, r.length_ms AS length_ms, coalesce(rp.score, 0) AS popularity FROM catalog.recording r JOIN catalog.artist a ON a.id = r.artist_id LEFT JOIN catalog.release_group rg ON rg.id = r.release_group_id LEFT JOIN catalog.recording_popularity rp ON rp.recording_id = r.id WHERE a.name ILIKE @p0 ESCAPE '\' AND rp.score IS NOT NULL ORDER BY rp.score DESC NULLS LAST LIMIT @row_limit) UNION ALL (SELECT r.mbid AS id, r.name AS title, a.name AS artist, rg.name AS album, r.first_release_year AS year, r.length_ms AS length_ms, coalesce(rp.score, 0) AS popularity FROM catalog.recording r JOIN catalog.artist a ON a.id = r.artist_id LEFT JOIN catalog.release_group rg ON rg.id = r.release_group_id LEFT JOIN catalog.recording_popularity rp ON rp.recording_id = r.id WHERE a.name ILIKE @p0 ESCAPE '\' AND rp.score IS NULL LIMIT @row_limit)) ranked LIMIT @row_limit
            --
            @p0 = text "%50\%\_off%"
            @row_limit = int 100
            """,
    ...
    public void Genre_inequality_negates_membership_across_all_levels()
    {
        ...
        Assert.Equal(
            """
            SELECT * FROM ((SELECT r.mbid AS id, r.name AS title, a.name AS artist, rg.name AS album, r.first_release_year AS year, r.length_ms AS length_ms, coalesce(rp.score, 0) AS popularity FROM catalog.recording r JOIN catalog.artist a ON a.id = r.artist_id LEFT JOIN catalog.release_group rg ON rg.id = r.release_group_id LEFT JOIN catalog.recording_popularity rp ON rp.recording_id = r.id WHERE (NOT (EXISTS (SELECT 1 FROM catalog.recording_genre lg JOIN catalog.genre g ON g.id = lg.genre_id WHERE lg.recording_id = r.id AND lower(g.name) = lower(@p0)) OR EXISTS (SELECT 1 FROM catalog.release_group_genre lg JOIN catalog.genre g ON g.id = lg.genre_id WHERE lg.release_group_id = r.release_group_id AND lower(g.name) = lower(@p0)) OR EXISTS (SELECT 1 FROM catalog.artist_genre lg JOIN catalog.genre g ON g.id = lg.genre_id WHERE lg.artist_id = r.artist_id AND lower(g.name) = lower(@p0)))) AND rp.score IS NOT NULL ORDER BY rp.score DESC NULLS LAST LIMIT @row_limit) UNION ALL (SELECT r.mbid AS id, r.name AS title, a.name AS artist, rg.name AS album, r.first_release_year AS year, r.length_ms AS length_ms, coalesce(rp.score, 0) AS popularity FROM catalog.recording r JOIN catalog.artist a ON a.id = r.artist_id LEFT JOIN catalog.release_group rg ON rg.id = r.release_group_id LEFT JOIN catalog.recording_popularity rp ON rp.recording_id = r.id WHERE (NOT (EXISTS (SELECT 1 FROM catalog.recording_genre lg JOIN catalog.genre g ON g.id = lg.genre_id WHERE lg.recording_id = r.id AND lower(g.name) = lower(@p0)) OR EXISTS (SELECT 1 FROM catalog.release_group_genre lg JOIN catalog.genre g ON g.id = lg.genre_id WHERE lg.release_group_id = r.release_group_id AND lower(g.name) = lower(@p0)) OR EXISTS (SELECT 1 FROM catalog.artist_genre lg JOIN catalog.genre g ON g.id = lg.genre_id WHERE lg.artist_id = r.artist_id AND lower(g.name) = lower(@p0)))) AND rp.score IS NULL LIMIT @row_limit)) ranked LIMIT @row_limit
            --
            @p0 = text "pop"
            @row_limit = int 100
            """,
